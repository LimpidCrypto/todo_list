...
def post_new_list() -> Tuple[Union[str, Dict[str, str]], int]:
    ...
    Returns:
        Tuple[Union[str, Dict[str, str]], int]: A tuple containing the new todo
          list or an error message and status code.
    """
    try:
...
def get_all_lists() -> Tuple[List[Dict[str, str]], int]:
    ...
    Returns:
        Tuple[List[Dict[str, str]], int]: A tuple containing a list of todo
            lists or an error message and status code.
    """
    try:
...
def delete_list_by_id(list_id: str) -> Tuple[str, int]:
    ...
    Returns:
        Tuple[str, int]: A tuple containing an error message and status code
            if the todo list does not exist, otherwise an empty string and
            status code.
    """
    try:
...
LIST_BLUEPRINT.add_url_rule(
    "/list/<list_id>", view_func=delete_list_by_id, methods=["DELETE"]
)
