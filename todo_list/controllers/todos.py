...
from todo_list.models import create_todo, NewTodoModel, find_list_by_id, find_todo_list_items, find_todo_by_id, edit_todo_list_item, remove_todo_list_item
from todo_list.controllers.validations import validate_list_existence, validate_todo_existence
...
def patch_todo_list_item(list_id: str, item_id: str):
    try:
        ...
        if is_error:
            return is_error
        todo_model = str(request.json).replace("'", '"')
        new_item = edit_todo_list_item(list_uuid, list_item_uuid, from_json(NewTodoModel, todo_model))
...
TODO_BLUEPRINT.add_url_rule('/list/<list_id>', view_func=get_todo_list_items, methods=['GET'])
TODO_BLUEPRINT.add_url_rule('/list/<list_id>/item', view_func=post_new_todo, methods=['POST'])
TODO_BLUEPRINT.add_url_rule('/list/<list_id>/item/<item_id>', view_func=patch_todo_list_item, methods=['PATCH'])
TODO_BLUEPRINT.add_url_rule('/list/<list_id>/item/<item_id>', view_func=delete_todo_list_item, methods=['DELETE'])
